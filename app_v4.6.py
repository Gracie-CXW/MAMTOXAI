...
def parse_snp(state:GraphState):
    ...
    modified_name = f"{pdf_name}_searchandparse"

    keys = list(filter(lambda key: vector_dict[key] == pdf_name, vector_dict))
    if len(keys) < 1: # No matching pdf to vector store with snp function
        print(f"Parsing Note (SnP): No Instances Found: I will now create a new embedding for pdf {pdf_name} within the directory {vectordb_dir}.")
        vector_store,persist_directory = search_and_parse(pdf_fp,state['keywords'],vectordb_dir)
        save_vector_store(
...
def parse_all(state:GraphState):
    ...
    if len(keys) < 1: # No matching pdf to vector store with jp function
        print(f"Parsing Note (JP): No Instances Found: I will now create a new embedding for pdf {pdf_name} within the directory {vectordb_dir}.")
        vector_store,persist_directory = just_parse(pdf_fp,vectordb_dir)
        save_vector_store(
            persist_directory = persist_directory,
            original_doc = pdf_name
        )
