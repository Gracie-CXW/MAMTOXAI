...
def save_vector_store(persist_directory:str,collection_name:str,original_doc:str,vector_save_fp:str):
    """
    Input Arguments:
    ...
    persist_directory (key) -> str : [original_doc,collection_name] (value) -> List(str)
    """
    try:
        with open(vector_save_fp, 'a') as save:
            save.write(f'{persist_directory}:[{original_doc},{collection_name}]\n')
...
def load_dict_from_txt(save_fp):
    ...
    import os
    """
    Loads a dictionary from a .txt file where each line is:
    key:[pdf_name, collection_name]
            ...
            if ':' in line:
                ...
                try:
                    ...
                    if isinstance(parsed_value, list):
                        ...
                        print(f"Warning: Value for key '{key}' is not a list.")
