...
def just_parse(pdf:str,vector_dir:str):
    ...
    try:
      ...
      ids = [idx for idx in range(len(pages))]
      ...
      vector_store_infos.append(persist_directory,collection_name,ids)
...
def search_and_parse(pdf: str, keywords: list[str], vector_dir: str,cache_dir):
    """
    Input Arguments:
    PDF file path: str
    ...
    Allows multiple searches to be performed!
    ...
    keywords_name = "_".join(keywords_n)
    modified_name = f"{pdf_name}_{keywords_name}_searchandparse" # Includes inclusion methond AND search method! yay
    persist_directory = os.path.join(vector_dir,modified_name)

    inclusion_method = keywords_n[0].strip().lower()
    parse_method = keywords_n[1].strip().lower()
    keywords_lower = [k.strip().lower() for k in keywords_n[2:]]

    # Initialize emebedding
    # need to download embedding function bc for some proxy error for some reason.
    embedding_func = HuggingFaceEmbeddings(model_name=r".\embeddings_local\all-MiniLM-L6-v2")
    print(f"Starting search and parse function on pdf '{pdf_name}', using methods '{inlcusion_method}' - '{parse_method}'.")
    ...
    if inclusion_method == 'and':
        if parse_method == 'literal':
            ...
            try: 
                ...
                full_vector_store = Chroma(collection_name = str(uuid.uuid4()),embedding_function = embedding_func,persist_directory=cache_dir)
                if full_docs:
                ...
                for k in keywords_lower:
                    ...
                    page_ids = [doc.metadata.get('page') for doc in relevant_k_docs]
                    relevant_page_nums.append(*page_ids)
                ...
                if final_pages:
                    vector_store.add_documents(documents=final_pages,ids)
    ...
    # for the 'OR' inclusion method
        ...
        if parse_method == 'literal':
            ...
            try: 
                    ...
                    for k in keywords_lower:
                        relevant_k_docs = full_vector_store.similarity_search(k)
                        relevant_docs.append(relevant_k_docs)
                        ids_k = [d.metadata.get('page') for d in relevant_docs]
                        ids.append(*ids_k)

                    if relevant_docs:
                        vector_store.add_documents(documents=relevant_docs,ids)
                    ...
                    if toc:
                        ...
                        for i in range(len(sections)):
                            ...
                            if all(k in combined_text for k in keywords_lower):
                                ...
                                relevant_pages.append(*combined_pages)
                    else:
