...
def save_vector_store(persist_directory:str,original_doc:str):
    """
    Input Arguments:
    ...
    persist_directory (key):original_doc (value)
    """
    try:
        with open(vector_save_fp, 'a') as save:
            save.write(f'{persist_directory}:{original_doc}\n')
...
def load_dict_from_txt(save_fp):
    """
    Input Arguments:
    Save filepath: filepath for the .txt file
    ...
    with open(save_fp, 'r') as f:
        if [line for line in f][0] is None:
