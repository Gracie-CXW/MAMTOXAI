...
def parse_snp(state:GraphState):
    ...
    if len(keys) < 1: # No matching pdf to vector store with snp function
        ...
        vector_store,persist_directory,collection_name = search_and_parse(pdf_fp,state['keywords'],vectordb_dir)
        save_vector_store(
...
def parse_all(state:GraphState):
    ...
    if len(keys) < 1: # No matching pdf to vector store with jp function
        ...
        vector_store,persist_directory,collection_name = just_parse(pdf_fp,vectordb_dir)
        save_vector_store(
            persist_directory = persist_directory,
            original_doc = pdf_name,
            collection_name = collection_name,
